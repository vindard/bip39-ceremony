...
pub enum Command {
    ...
    RecordCoinFourD6Coin(CoinFlip),
    RecordCoinFourD6D6(DieFace),
    RecordD20(D20Face),
    UndoRoll,
    UndoFlip,
    UndoJade,
...
impl fmt::Debug for Command {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::RecordCoinFourD6Coin(_) => {
                formatter.write_str("RecordCoinFourD6Coin([REDACTED])")
            }
            Self::RecordCoinFourD6D6(_) => formatter.write_str("RecordCoinFourD6D6([REDACTED])"),

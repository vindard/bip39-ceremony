...
fn timeline_entry(position: usize, event: &Event) -> TimelineEntry {
    let (description, secret_bearing) = match event {
        ...
        Event::JadeD16Recorded(_) => ("recorded secret D16 roll".to_owned(), true),
        Event::JadeD8Recorded(_) => ("recorded secret D8 roll".to_owned(), true),
        Event::BitBoxD6Recorded(_) => ("recorded secret BitBox D6 roll".to_owned(), true),
        Event::BitBoxCoinRecorded(_) => ("recorded secret BitBox coin side".to_owned(), true),
        Event::CoinFourD6CoinRecorded(_) => {
            ("recorded secret direct-table coin side".to_owned(), true)
        }
        Event::CoinFourD6D6Recorded(_) => ("recorded secret direct-table D6 roll".to_owned(), true),
        Event::D20Recorded(_) => ("recorded secret D20 roll".to_owned(), true),
        ...
        Event::JadeUndone => ("undid the latest mixed-dice roll".to_owned(), true),
        Event::BitBoxUndone => ("undid the latest BitBox observation".to_owned(), true),
        Event::CoinFourD6Undone => ("undid the latest direct-table observation".to_owned(), true),
        Event::D20Undone => ("undid the latest D20 roll".to_owned(), true),
        Event::RollsConfirmed => ("confirmed physical transcription".to_owned(), true),
